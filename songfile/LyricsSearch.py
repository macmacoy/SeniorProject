    ...
    if isinstance(xml["return"]["fileinfo"], list):
      for item in xml["return"]["fileinfo"]:
        ...
        results.append({'artist': artist, 'title': title, 'rating': float(rating),
                'rating_count': int(rating_count), 'downloads': int(downloads), # add numbers of ratings/downloads
      ...
      # BEGIN MODIFICATIONS
      try:
        rating_count = xml["return"]["fileinfo"]["@ratecount"]
      except:
        rating_count = 0
      try:
        downloads = xml["return"]["fileinfo"]["@downloads"]
      except:
        downloads = 0
      # END MODIFICATIONS
      try:
