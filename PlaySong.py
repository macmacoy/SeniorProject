...
lyricPlacement = (screenSize[0]/2, screenSize[1]/12)
feedbackSize = (int(screenSize[1]/3), int(screenSize[1]/3))
feedbackPlacement = (screenSize[0]/12, screenSize[0]/8)
currentChordImagePlacement = (screenSize[0]/2, screenSize[1]/5)
...
def scaleForNextChord(image):
	return pygame.transform.scale(image, (int(image.get_width()/1.2), int(image.get_height()/1.2)))
...
while now < song.duration:
	...
	if(chordIndex != len(chords)-1):
		...
		scaledNextChordImagePlacement = (nextChordImagePlacement[0] - scaledNextImageChord.get_rect().width/2, nextChordImagePlacement[1] + scaledNextImageChord.get_rect().height/2)
